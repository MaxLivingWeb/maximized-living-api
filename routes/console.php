...
Artisan::command('fixwholesaler {file}', function ($file) {
    try {
        $regions = [
            'AL' => 'Alabama',
            'AK' => 'Alaska',
        ...
        foreach ($csv as $user) {
            ...
            $cognitoUser = $cognito->getUser($email);

            if (is_null($cognitoUser)) {
                dd('error, no cognito user found for ' . $email);
            }

            $group = \App\UserGroup::where('legacy_affiliate_id', $legacy_affiliate_id)->first();
            if (!is_null($group)) {
                $group->wholesaler = true;
                $group->save();
            }
            else {
                $params = [
                ...
                $newWholesaleGroup = \App\UserGroup::create($params);
                $newWholesaleGroup->addUser($cognitoUser['Username']);
